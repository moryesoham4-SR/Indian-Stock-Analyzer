...
st.markdown(
    ...
    /* ── ALL INPUT ELEMENTS — NSE Terminal Style ── */
    ...
    .stTextInput > div > div > input:focus {
        border-color: #00ff80 !important;
        box-shadow: 0 0 8px rgba(0,255,80,0.2) !important;
    ...
    .stRadio label {
        background: #010e07 !important;
        border: 1px solid #004d22 !important;
        border-radius: 2px !important;
    ...
    @media (max-width: 768px) {
        ...
        /* Multiselect tags on mobile */
        [data-baseweb="tag"] span {
            font-size: 11px !important;
        }

        /* Radio buttons stacked on mobile */
        .stRadio > div {
...
if page == "Dashboard":
    ...
    st.title(f"📈 {selected_stock} — Live Dashboard")
    ...
    period_map = {"7 Days": 7, "30 Days": 30, "6 Months": len(data)}
    n_days     = period_map[period_tab]
    chart_data = data.tail(n_days).copy()
    ...
    with st.spinner("Rendering chart..."):
        ...
        if period_tab != "7 Days":
            fig.add_trace(go.Scatter(x=chart_data["Date"], y=chart_data["MA20"], mode="lines", name="MA20", line=dict(color="#FBBF24", width=1.5)))
        if period_tab == "6 Months":
            fig.add_trace(go.Scatter(x=chart_data["Date"], y=chart_data["MA50"], mode="lines", name="MA50", line=dict(color="#60A5FA", width=1.5)))
        fig.update_layout(**mobile_layout(
            title=f"{selected_stock} — {period_tab} Candlestick",
    ...
    if len(data) >= 7:
        ...
        last7   = data.tail(7)
        w_open  = float(last7["Open"].iloc[0])
        w_close = float(last7["Close"].iloc[-1])
        w_high  = float(last7["High"].max())
        w_low   = float(last7["Low"].min())
        w_chg   = ((w_close - w_open) / w_open) * 100
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("7D Open",   f"₹{w_open:.2f}")
        c2.metric("7D Close",  f"₹{w_close:.2f}", f"{w_chg:+.2f}%")
        c3.metric("7D High",   f"₹{w_high:.2f}")
        c4.metric("7D Low",    f"₹{w_low:.2f}")

    # ── 30 Day Summary ──
    if len(data) >= 30:
        st.subheader("📊 30-Day Performance Summary")
        last30  = data.tail(30)
        m_open  = float(last30["Open"].iloc[0])
        m_close = float(last30["Close"].iloc[-1])
        m_high  = float(last30["High"].max())
        m_low   = float(last30["Low"].min())
        m_chg   = ((m_close - m_open) / m_open) * 100
        m_vol   = int(last30["Volume"].mean())
        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("30D Open",    f"₹{m_open:.2f}")
        c2.metric("30D Close",   f"₹{m_close:.2f}", f"{m_chg:+.2f}%")
        c3.metric("30D High",    f"₹{m_high:.2f}")
        c4.metric("30D Low",     f"₹{m_low:.2f}")
        c5.metric("Avg Volume",  f"{m_vol:,}")
...
# =========================
    ...
    # Static data — always shows instantly, no rate limit
    static_data = [
        {"Stock": "RELIANCE",   "Price": 2943.20, "Change %": 1.24},
        {"Stock": "TCS",        "Price": 3821.00, "Change %": 0.82},
        {"Stock": "INFY",       "Price": 1821.45, "Change %": 1.75},
        {"Stock": "HDFCBANK",   "Price": 1634.80, "Change %": 2.10},
        {"Stock": "ICICIBANK",  "Price": 1142.30, "Change %": 1.52},
        {"Stock": "ITC",        "Price": 462.15,  "Change %": 0.68},
        {"Stock": "SBIN",       "Price": 812.40,  "Change %": 1.10},
        {"Stock": "BHARTIARTL", "Price": 1876.25, "Change %": 0.95},
        {"Stock": "LT",         "Price": 3412.60, "Change %": -0.45},
        {"Stock": "TATAMOTORS", "Price": 924.35,  "Change %": 3.21},
        {"Stock": "ADANIGREEN", "Price": 1234.50, "Change %": -0.92},
    ]
    ...
    if fetch_live:
        ...
        if live_data:
            ...
            st.success(f"✅ Live data fetched for {len(live_data)} stocks!")
        else:
            st.warning("⚠️ Rate limited — showing last known data. Try again in 2 minutes.")
    ...
    colors = ["#00ff80" if c > 0 else "#EF4444" for c in market_df["Change %"]]
...
# =========================
    ...
    if watchlist:
        watch_data  = []
        fetch_watch = st.button("🔄 Fetch Watchlist Prices")

        if not fetch_watch:
            st.info("Click **Fetch Watchlist Prices** to load current prices.")
        else:
            progress_bar = st.progress(0)
            for idx, symbol in enumerate(watchlist):
                try:
                    ...
                    if hist is not None and len(hist) >= 2:
                        ...
                        watch_data.append({
                            "Stock":         symbol.replace(".NS", ""),
                            "Current Price": "N/A", "Prev Close": "N/A",
                            "Change %":      0,     "Day High":   "N/A",
            ...
            progress_bar.empty()
            if not watch_data:
                st.warning("⚠️ Rate limited. Try again in 2 minutes.")
            else:
