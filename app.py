...
# =========================
    ...
    st.markdown("Deep Learning model trained on historical price data to forecast future prices.")
    ...
    with col_b:
        lookback = st.selectbox("Lookback Window (days)", [30, 60, 90], index=1)
    with col_c:
        epochs = st.selectbox("Training Epochs", [10, 20, 50], index=0)

    run_lstm = st.button("🚀 Train & Predict")
    ...
    if run_lstm:
        ...
        try:
            ...
            # ── FETCH 2 YEARS DATA for better training ──
            with st.spinner("Fetching 2 years of price data..."):
                lstm_data = ticker.history(period="2y")
                lstm_data.reset_index(inplace=True)

            if len(lstm_data) < lookback + 10:
                st.error("Not enough data to train. Try a smaller lookback window.")
                st.stop()

            close_prices = lstm_data["Close"].values.reshape(-1, 1)
            ...
            # ── BUILD LSTM MODEL ──
            bar = st.progress(0)
            st.info("🧠 Building LSTM Neural Network...")

            import uuid
            uid = uuid.uuid4().hex[:6]
            model = Sequential([
                LSTM(64, return_sequences=True, input_shape=(lookback, 1), name=f"lstm1_{uid}"),
                Dropout(0.2, name=f"drop1_{uid}"),
                LSTM(64, return_sequences=False, name=f"lstm2_{uid}"),
                Dropout(0.2, name=f"drop2_{uid}"),
                Dense(32, activation="relu", name=f"dense1_{uid}"),
                Dense(1, name=f"output_{uid}")
            ])
            ...
            # ── TRAIN ──
            st.info(f"⚙️ Training for {epochs} epochs... (may take 30–60 seconds)")
            ...
            actual_prices = scaler.inverse_transform(scaled[lookback:])

            # RMSE
            rmse = float(np.sqrt(np.mean((train_predict.flatten() - actual_prices.flatten()) ** 2)))
            ...
            c4.metric("📉 RMSE Error",           f"₹{rmse:.2f}")

            if expected_change > 1:
                st.success("🟢 LSTM predicts an UPWARD move — Bullish signal")
            elif expected_change < -1:
                st.error("🔴 LSTM predicts a DOWNWARD move — Bearish signal")
            else:
                st.warning("🟡 LSTM predicts a SIDEWAYS move — Neutral signal")

            # ── MAIN PREDICTION CHART ──
            st.subheader("📊 LSTM Prediction vs Actual Price")

            actual_dates  = lstm_data["Date"].iloc[lookback:].reset_index(drop=True)
            future_dates  = [
            ...
            pred_fig.add_trace(go.Scatter(
                x=actual_dates, y=actual_prices.flatten(),
                mode="lines", name="Actual Price",
                line=dict(color="#22C55E", width=2)
            ))

            pred_fig.add_trace(go.Scatter(
                x=actual_dates, y=train_predict.flatten(),
            ...
            pred_fig.update_layout(**mobile_layout(
                title=f"LSTM Forecast — Next {forecast_days} Days",
                xaxis_title="Date", yaxis_title="Price (₹)",
                height=520
            ))
            ...
            loss_fig.update_layout(**mobile_layout(
                title="Loss per Epoch (lower = better)",
            ...
            forecast_df = pd.DataFrame({
                "Date":               future_dates,
                "Predicted Price (₹)": [round(p, 2) for p in future_prices],
                "Change from Today":  [f"{((p - current_p)/current_p)*100:+.2f}%" for p in future_prices]
            })
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"{stock}_lstm_forecast.csv",
                mime="text/csv"
            )
            ...
            st.info("After installing, restart Streamlit and try again.")

    else:
        st.info("Choose your settings above and click **Train & Predict** to run the LSTM model.")
        st.markdown("""
        **How LSTM works:**
        - Looks at the last N days of prices (lookback window)
        - Learns patterns — trends, cycles, momentum
        - Predicts the next price based on those patterns
...
# =========================
    ...
    if watchlist:
        ...
        for idx, symbol in enumerate(watchlist):
            try:
                si            = yf.Ticker(symbol)
                current_price = si.info.get("currentPrice", 0)
                prev_close    = si.info.get("previousClose", current_price)
